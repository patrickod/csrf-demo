...
// TLSContextKey is the context key used to store whether the request is being
// served via TLS.
// This determines whether to enforce strict referer checking and is intended to
// prevent form-based CSRF attacks from untrusted origins.
// The caller is responsible for setting this depending on the circumstances of
// their environment and whether the Go process is terminating TLS itself or
// accepting trusted headers from reverse proxies.
const TLSContextKey contextKey = "tls"
...
func (cs *csrf) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if !contains(safeMethods, r.Method) {
		// determine whether the request is being served via TLS by checking the
		// request context.
		var isTLS bool

		val := r.Context().Value(TLSContextKey)
		if val != nil {
			isTLS, _ = val.(bool)
		}
		...
		if origin != "" {
			...
			if !sameOrigin(requestURL, parsedOrigin) && !cs.isTrustedOrigin(parsedOrigin.Host) {
				r = envError(r, ErrBadOrigin)
				cs.opts.ErrorHandler.ServeHTTP(w, r)
				return
			}
		...
		if origin == "" && isTLS {
			...
			if referer.Host != "" && referer.Host != r.Host && !cs.isTrustedOrigin(referer.Host) {
				r = envError(r, ErrBadReferer)
				cs.opts.ErrorHandler.ServeHTTP(w, r)
				return
			}
...
func (cs *csrf) isTrustedOrigin(origin string) bool {
